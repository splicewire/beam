...
const AccountHome = lazy(async () => ({ default: await resolveBeamPage('account/home') }));
const OperatorDashboard = lazy(async () => ({ default: await resolveBeamPage('operator/dashboard') }));
const SiteHome = lazy(async () => ({ default: await resolveBeamPage('site/home') }));
...
export function OsShellDesktop() {
    ...
    const devForceAppFirst =
        getBeamInertiaConfig().development &&
        typeof window !== 'undefined' &&
        new URLSearchParams(window.location.search).get('os') === 'off';

    if ((!canEnterOs && !getBeamInertiaConfig().development) || devForceAppFirst) {

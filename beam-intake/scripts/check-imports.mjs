#!/usr/bin/env node
/**
 * Static import-boundary gate (rehome-components §8b — the deny-list enforcer).
 *
 ...
 * it runs anywhere and lifts cleanly into ticket 07's agentic convention. Twin of the gate
 * in @schemastud/ui.
 */
...
// component must never reach for. Mirrors the rehome-components contract §6.
const FORBIDDEN = [
    { re: /@splicewire\/splice/, why: 'upward paid-domain dependency' },
    {
        re: /--splice-|--swc-|#[0-9a-fA-F]{3,8}\b/,
        why: 'wrong-tier or raw color token',
    },
    { re: /from\s+['"]@\//, why: "app-local '@/…' import" },
    { re: /import\s+['"]@\//, why: "app-local '@/…' side-effect import" },
    {
        re: /from\s+['"]sonner['"]/,
        why: 'direct toast lib (feedback is injected, not imported)',
    },
    {
        re: /from\s+['"]axios['"]/,
        why: 'transport lib (the client is injected, not imported)',
    },
    {
        re: /from\s+['"]ziggy-js['"]/,
        why: 'named-route resolution has no place in a portable component',
    },
...
if (violations.length) {
    console.error(
        '✗ import-boundary check FAILED — @splicewire/beam-intake must stay host-agnostic:\n',
    );
